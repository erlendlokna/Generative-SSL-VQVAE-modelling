...
from models.stage2.transformers import PoolBidirectionalTransformer
...
class SSLMaskGIT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.transformer = PoolBidirectionalTransformer(
            self.num_tokens,
        ...
        # latent_ssl_method = assign_ssl_method()
    ...
    def forward(self, x, y):
        ...
        # create masks
        mask = self.generate_mask(s.shape, device)  # (b n)
        ...
        s_M_complement = (~mask) * s + mask * masked_indices  # (b n)
        # (b n); `~` reverses bool-typed data

        # prediction
        logits_regular, latents_regular = self.transformer(
            s_M.detach(), class_condition=y, mask=mask
        )  # (b n codebook_size)

        logits_complement, latents_complement = self.transformer(
            s_M_complement.detach(), class_condition=y, mask=~mask
        )

        logits = [logits_regular, logits_complement]
        latents = [latents_regular, latents_complement]
        target = s  # (b n)

        return logits, latents, target

    def generate_mask(self, s_shape, device):

        n_masks = 0
        while True:
            t = np.random.uniform(0, 1)
            n_masks = math.floor(self.gamma(t) * (s_shape[1]))
            if n_masks < s_shape[1] and n_masks > 0:
                break
        rand = torch.rand(s_shape, device=device)  # (b n)
        mask = torch.zeros(s_shape, dtype=torch.bool, device=device)
