...
class SSL_wrapper(nn.Module):
    def __init__(self, proj_in, config):
        super().__init__()
        ...
        self.name = config["SSL"]["method_choice"]

        assert (
            self.name in method_mapping
        ), f"SSL method {self.name} not in choices {list(method_mapping.keys())}"

        self.loss_weighting = config["SSL"]["weighting"]

        self.SSL_lossfunc = method_mapping[self.name](proj_in, config)

    def forward(self, z1, z2):
        return self.SSL_lossfunc(z1, z2)
...
class BarlowTwins(nn.Module):
    def __init__(self, proj_in, config):
        ...
        self.projector = Projector(
            proj_in=proj_in,
            proj_hid=SSL_config["proj_hid"],
            proj_out=SSL_config["proj_out"],
        )
