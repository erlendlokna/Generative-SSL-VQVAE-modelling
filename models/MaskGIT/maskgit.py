...
class MaskGIT(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = VQVAEDecoder(dim, 2 * in_channels, downsample_rate, config['decoder']['n_resnet_blocks'])
    ...
    def forward(self, x, y):
        ...
        # masked tokens
        masked_indices = self.mask_token_ids * torch.ones_like(s, device=device)  # (b n)
        s_M = mask * s + (~mask) * masked_indices  # (b n); `~` reverses bool-typed data

        # prediction
        logits = self.transformer(s_M.detach(), class_condition=y)  # (b n codebook_size)
        target = s  # (b n)
    ...
    def sample_indices(self,
        ...
        for t in range(self.T):
            logits = self.transformer(s, class_condition=class_condition)  # (b n codebook_size) == (b n K)
    ...
    def decode_token_ind_to_timeseries(self, s: torch.Tensor, return_representations: bool = False):
        ...
        quantize = rearrange(quantize, 'b c (h w) -> b c h w', h=self.H_prime, w=self.W_prime)
