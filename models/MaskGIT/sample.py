...
def unconditional_sample(maskgit: MaskGIT, n_samples: int, device, class_index=None, batch_size=256, return_representations=False, guidance_scale=1.):
    ...
    for i in range(n_iters):
        ...
        embed_ind = sample_callback(num=b, device=device, class_index=class_index, guidance_scale=guidance_scale)
...
def conditional_sample(maskgit: MaskGIT, n_samples: int, device, class_index: int, batch_size=256, return_representations=False, guidance_scale=1.):
    ...
    """
    maskgit.transformer.p_unconditional = 0.

    if return_representations:
        x_new, quantize_new = unconditional_sample(maskgit, n_samples, device, class_index, batch_size, guidance_scale=guidance_scale)
        return x_new, quantize_new
    else:
        x_new = unconditional_sample(maskgit, n_samples, device, class_index, batch_size, guidance_scale=guidance_scale)
        return x_new
