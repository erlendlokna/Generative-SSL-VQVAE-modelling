...
def load_args():
    ...
    parser.add_argument(
        "--config_cas",
        type=str,
        help="Path to the config_cas data  file.",
    ...
    parser.add_argument(
        "--gpu_device_idx", type=int, help="GPU device index", default=0
    )
    parser.add_argument(
        "--min_n_synthetic_train_samples",
        type=int,
        default=1000,
    ...
    parser.add_argument(
        "--n_dataset_partitions",
        type=int,
        default=1,
        help="used to partition all the subset datasets into N groups so that each group can be run separately.",
    )
    parser.add_argument(
        "--partition_idx",
        default=0,
        type=int,
        help="selects one partitions among the N partitions; {0, 1, ..., n_dataset_partitions-1}",
    )
    parser.add_argument(
        "--data_summary_ucr_condition_query",
        type=str,
        default="",
        help="query to condition `data_summary_ucr_condition_query` to narrow target subset datasets.",
    )
    return parser.parse_args()


def get_target_ucr_dataset_names(
    args,
):
    if args.dataset_names:
        ...
        if args.data_summary_ucr_condition_query:
            dataset_names = data_summary_ucr.query(
                args.data_summary_ucr_condition_query
            )[
                "Name"
            ].values  # e.g., "500 <= Train <= 1000" for `data_summary_ucr_condition_query`
        if args.n_dataset_partitions == 1:
            ...
            dataset_names = dataset_names[
                args.partition_idx * b : (args.partition_idx + 1) * b
            ]
        else:
            raise ValueError
...
if __name__ == "__main__":
    ...
    for dataset_name in dataset_names:
        ...
        wandb_logger = WandbLogger(
            project="TimeVQVAE-CAS",
            name=config["dataset"]["dataset_name"],
            config=vars(args) | config_cas | config,
        )
        trainer = pl.Trainer(
            logger=wandb_logger,
