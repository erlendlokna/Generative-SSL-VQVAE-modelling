...
def train_SSL_VQVAE(
    ...
    train_model = Exp_SSL_VQVAE(
        input_length, config=config, n_train_samples=len(train_data_loader.dataset)
    )
    ...
    n_trainable_params = sum(
        p.numel() for p in train_model.parameters() if p.requires_grad
    )
    ...
    save_model(
        {
            f"{SSL_method}_{SSL_weigting}_encoder": train_model.encoder,
...
if __name__ == "__main__":
    ...
    train_data_loader_non_aug, test_data_loader = [
    ...
    train_SSL_VQVAE(
        config,
        aug_train_data_loader=train_data_loader_aug,
